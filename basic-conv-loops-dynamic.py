...
for i in range(1,3):
    with tf.variable_scope(str(i)):
        n_prev = int((x_list[i-1]).get_shape()[3])
        w = weight_variable([5,5,n_prev,n_new])
...
shape = x_list[-1].get_shape()
nodes = int(shape[1])*int(shape[2])*int(shape[3])
w_3 = weight_variable([nodes, 1024])
b_3 = bias_variable([1024])

flatten = tf.reshape(x_list[-1], [-1, nodes])
